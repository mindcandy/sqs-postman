...
var SEND_MESSAGE_PARALLEL_LIMIT = 2;
...
function create(options) {
  ...
  return new Postman(sqs);
}

function Postman(sqs) {
  this.sqs = sqs;
};
...
Postman.prototype.sendMessage = function (options, cb) {
  ...
  this.getQueueUrl(queueName, function (err, queueUrl) {
    ...
    async.eachLimit(batches, SEND_MESSAGE_PARALLEL_LIMIT, function sendMessageBatch(batch, next) {
      debug('sending message batch %s', batch);
      ...
      self.sqs.sendMessageBatch({ Entries: entries, QueueUrl: queueUrl }, function (err, data) {
        ...
        debug('successfully processed batch %s', batch);

        next(null);
      })
    }, function (err) {
      console.log('batches done %j', batches);
      cb(null, 'done');
    });    
  });
};
